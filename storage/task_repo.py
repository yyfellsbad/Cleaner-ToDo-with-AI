...
_COLUMNS = "id, name, date, end_date, description, completed"
...
class TaskRepository:
    ...
    def create_task(self, task: TaskRecord) -> TaskRecord:
        with transaction(self.db_path) as conn:
            cursor = conn.execute(
                "INSERT INTO tasks (name, date, end_date, description, completed) VALUES (?, ?, ?, ?, ?)",
                task.to_db_values(),
            )
    ...
    def update_task(self, task: TaskRecord) -> TaskRecord:
        ...
        with transaction(self.db_path) as conn:
            conn.execute(
                "UPDATE tasks SET name=?, date=?, end_date=?, description=?, completed=? WHERE id=?",
