...
- VSCode-style left sidebar (48px): icon buttons for chat, stats, calendar, and settings. Selected item has PRIMARY color background highlight. Chat drawer toggles independently; stats, calendar, and settings are mutually exclusive content views.
- Chat drawer: side panel in a `ft.Row` layout (sidebar → drawer → content), uses `animate_opacity` for fade transition. Rounded corners: `ft.BorderRadius(12, 4, 4, 12)`. Shadow: `blur_radius=8, opacity=0.1, offset=2` (soft). Chat history `bgcolor=ft.Colors.SURFACE`. Assistant bubbles use `ft.Markdown` (GITHUB_WEB). Thinking animation with ProgressRing while LLM processes. On first open, assistant greets with time-based greeting + urgent task summary (ASCII art if no urgent tasks). Quick action Chips above input: "最近七天计划", "我接下来该做什么", "查看所有待办", "清除已完成". **Chat bubbles have entrance animation** (fade in + scale up).
- **Toast notifications**: task operations (add/delete/clear) show brief toast messages in bottom-right corner with fade-out animation.
- **Keyboard shortcuts**: Ctrl+Z (undo), Ctrl+N (focus new task input), Esc (close active panel/drawer).
